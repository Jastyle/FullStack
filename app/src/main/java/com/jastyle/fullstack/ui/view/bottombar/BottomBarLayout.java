package com.jastyle.fullstack.ui.view.bottombar;
...
public class BottomBarLayout extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private void init() {
        ...
        for (int i = 0; i < mChildCount; i++) {
            if (getChildAt(i) instanceof BottomBarItem) {
                ...
                if (i==2) {/*为tab_add单独添加点击效果*/
                    bottomBarItem.getImageView().setBackground(mViewPager.getContext().getResources().getDrawable(R.drawable.selector_bottom_tab_add));
                }
    ...
    public void onPageSelected(int position) {
        ...
        resetState();
        if (mCurrentItem>=2) {
            /*调用viewPager.setCurrentItem(index) 传递过来的index=(index>=2?index-1:index),so +1得到点击的index*/
            mItemViews.get(position+1).setStatus(true);
        }else {
    ...
    private class MyOnClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (currentIndex==2) {/*过滤掉add按钮，实际fragment数量为4*/
                return;
            }
            //点击前先重置所有按钮的状态
            resetState();
            ...
            //不能使用平滑滚动，否者颜色改变会乱
            if (currentIndex>2) {/*过滤掉add按钮，实际fragment数量为4*/
                mViewPager.setCurrentItem(currentIndex-1, mSmoothScroll);
            }else {
